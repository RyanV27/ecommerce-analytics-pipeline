...
-- Use the dataset's own max purchase date as the reference "today" so that
-- recency_days is meaningful for historical data ending in 2018.
bounds as (
...
customer_agg as (
    select
        ...
        date_diff((select max_purchase_date from bounds), max(date(order_purchase_timestamp)), day)          as recency_days,
